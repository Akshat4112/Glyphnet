...
def homo_gen_1(domain):
    """Sampling 1 approach for homoglyph generation"""
...
def homo_gen_2(domain):
    """Sampling 2 approach for homoglyph generation"""
...
## Dividing the dataset so that we can apply the multiprocessing.
...
if __name__ == "__main__":
    ...
    with Manager() as manager:
        ...
        data_tuples_1 = list(zip(domains_1, homo_1)) ## homoglyph from 1st processor
        data_tuples_2 = list(zip(domains_2, homo_2)) ## homoglyph from 2nd processor

        dataf_1 = pd.DataFrame(data_tuples_1, columns=['domain', 'homoglyphs'])
        dataf_2 = pd.DataFrame(data_tuples_2, columns=['domain', 'homoglyphs'])

        dataf = pd.concat([dataf_1, dataf_2]) ## Concatenating the data from 1st processor and 2nd processor

        dataf.to_csv('../data/dataset_final.csv', index=False) ## Now this csv is feed to ImageGeneration.py
