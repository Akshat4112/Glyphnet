...
class NeuralNetwork:
  ...
  def AttentionCNN(self):
    ...
    self.history = model.fit(self.train_it,
                    validation_data=self.validation_it, 
                    steps_per_epoch=500, 
                    epochs=50, 
                    batch_size=64, 
                    verbose=1, 
                    callbacks=[WandbCallback(data_type='image'), callback])
  ...
  def Evaluation(self):
    ...
    wandb.log({'Test error Rate': round(1-test_acc)*100})

    print("Computing Precision and Recall for Classification.")
    true_labels = self.test_it.classes
    predictions = self.model.predict_generator(self.test_it)
    y_true = true_labels
